...
from normalizing_flows.utils import Scale, StepOfGlow, ZeroConv2d
...
class GlowBlock(torch.nn.Module):
    ...
    def reverse(self, z_l,fixed_sample, seed, T=1., device=torch.device("cpu")):
        """reverse operation

        Args:
            z_l (torch.Tensor): tensor coming previous transformations
            T: (float) temperature used for sampling
            fixed_sample: if generated samples are fixed, in this case seed needs to be provided to fix the sample from Normal
